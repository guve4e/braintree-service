...
class MySql
{
    ...
    private function executeSqlQuery($sql)
    {
        ...
        if (is_null($result) || $result == false)
            throw new DatabaseException("Bad Query!: {$sql}");
    ...
    private function queryWrite($sql)
    {
        ...
        if ($this->connection->getAffectedRows() < 1)
            throw new DatabaseException("This Query Was not Successful: {$sql}");
     ...
     * @throws Exception
     */
    private function changeOption($sql)
    {
        $result = $this->executeSqlQuery($sql);

        if (!$result)
            throw new DatabaseException("Mysql was not able to change option: {$result}");
    }
